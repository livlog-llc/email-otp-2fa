...
IntelliJ IDEA + SmartTomcat など、クラスを `WEB-INF/classes` ではなくアプリのクラスパス（`target/classes` 以外）へ載せて配備するランタイムでは、Tomcat のデフォルト設定だとクラスパス上のアノテーションスキャンが無効になります。このサンプルでは、`WEB-INF/web.xml` でリスナーと Servlet を明示登録しているため、`@WebServlet` / `@WebListener` アノテーションに依存しません。少なくとも `/`, `/login`, `/mfa`, `/app` は必ずルーティングされます。
